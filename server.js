...
const ENEMY_POOLS = {
  ...
  mid: [
    {name:'Chaos Marauder',     type:'Chaos',   threat:'Moderate', hp:20, ac:14,atk:0,xp:2,gold:[10,25]},
    {name:'Skaven Stormvermin', type:'Skaven',  threat:'Moderate', hp:18, ac:15,atk:0,xp:2,gold:[8,20]},
    {name:'Wight',              type:'Undead',  threat:'Moderate', hp:28, ac:12,atk:0,xp:2,gold:[5,15],  undead:true,lifeLeech:true},
    {name:'Plague Monk',        type:'Chaos',   threat:'Moderate', hp:19, ac:13,atk:0,xp:2,gold:[0,0],   diseased:true},
  ],
  high: [
    {name:'Chaos Warrior',      type:'Chaos',   threat:'High',     hp:35, ac:16,atk:0,xp:3,gold:[15,40]},
    {name:'Vampire Count',      type:'Undead',  threat:'High',     hp:40, ac:15,atk:0,xp:3,gold:[20,60], undead:true,lifeLeech:true},
    {name:'Bloodletter',        type:'Daemon',  threat:'High',     hp:45, ac:15,atk:0,xp:3,gold:[25,50], insanityAtk:true},
    {name:'Skaven Warlord',     type:'Skaven',  threat:'High',     hp:32, ac:14,atk:0,xp:3,gold:[10,30], diseased:true},
  ],
  ...
  // Boss 2 (depth 20) — random
  boss2: [
    {name:'Varghulf',            type:'Undead Boss', threat:'Boss',hp:120,ac:15,atk:0,xp:5,gold:[40,100],undead:true,lifeLeech:true,regen:true},
    {name:'Bonebreaker Ratogre', type:'Daemon Boss', threat:'Boss',hp:120,ac:16,atk:0,xp:5,gold:[50,120],insanityAtk:true},
  ],
  // Boss 3 (depth 30) — always Saurian Ancient
  boss3: [
    {name:'Saurian Ancient',     type:'Ancient Boss',threat:'Boss',hp:200,ac:15,atk:0,xp:5,gold:[60,150]},
  ],
};
...
function enemyDmgDice(threat, isElite, bossCount) {
  ...
  // Post-boss-1: normal and elite both 2d6
  // Pre-boss-1: normal 1d6, elite 1d6+2
  if (bossCount === 0) {
    return isElite ? {n:1,s:6,b:2} : {n:1,s:6,b:0};
  }
  return {n:2,s:6,b:0}; // both normal and elite: 2d6 after first boss
}
...
function scaleEnemy(tmpl, playerCount, isElite, bossCount) {
  ...
  // All ATK bonuses are 0 — hits determined purely by d20 vs Defense
  e.atk = 0;
  return e;
}
...
function pickEnemy(depth, isElite, isBoss, playerCount, bossCount) {
  let pool;
  if (isBoss) {
    // Boss tier depends on how many bosses defeated so far
    if (bossCount===0)      pool=ENEMY_POOLS.boss1;
    else if (bossCount===1) pool=ENEMY_POOLS.boss2;
    else                    pool=ENEMY_POOLS.boss3;
  }
  else if (depth>20) pool=isElite?ENEMY_POOLS.boss1:ENEMY_POOLS.high;
...
function resolveEnemyDeath(room) {
  ...
  gs.playersActedThisRound=[]; gs.enemyHasActed=false;
  // Victory after defeating the 3rd boss (Saurian Ancient at depth 30)
  if(gs.bossCount>=3){gs.phase='victory';addLog(room,'🏆 The Saurian Ancient falls! The warband conquers the depths! FOR SIGMAR!','crit');}
}
...
function handlePlayerAction(room,playerId,payload,ws){
  ...
  if(action==='PRESS_ONWARD'){
    ...
    if(gs.depth>=30&&gs.bossCount>=3){gs.phase='victory';addLog(room,'🏆 The warband conquers the depths! FOR SIGMAR!','crit');return;}
