...
function maybeEnemyAttack(room) {
  ...
  alive.forEach(p=>{
    ...
    if(r.hit){
      ...
      addLog(room,`${e.name} hits <strong>${p.name}</strong> for <strong>${r.dmg}</strong> dmg (${r.total} vs Def ${p.char.defense+auraBonus})${r.crit?' — CRIT!':''}`,'dmg');
      ...
      addLog(room,`${e.name} attacks ${p.name} (${r.total} vs Def ${p.char.defense+auraBonus}) — miss.`,'sys');
    }
  });
  if(e.multi){
...
function handleBuy(room,player,itemId){
  ...
  if(item.type==='enhance'){
    // +1 dmg to the currently equipped weapon's bonus
    if(char.equippedWeapon){ char.equippedWeapon.bonus=(char.equippedWeapon.bonus||0)+1; addLog(room,`${player.name}: Weapon Enhancement — ${char.equippedWeapon.name} now +${char.equippedWeapon.bonus} dmg.`,'loot'); }
    else { char.weaponDmgBonus++; addLog(room,`${player.name}: Weapon Enhancement — +1 damage stored.`,'loot'); }
  }
...
function handlePlayerAction(room,playerId,payload,ws){
  ...
  if(action==='ATTACK'){
    ...
    if(r.fumble){addLog(room,`${player.name} fumbles (natural 1)!`,'sys');}
    else if(r.hit){
      gs.enemy.hp-=r.dmg;
      const cl=r.forceCrit?' — Lucky Pendant CRIT!':r.crit?' — CRITICAL HIT!':'';
      addLog(room,`${player.name} ${r.crit?'<strong>CRITS</strong>':'hits'} for <strong>${r.dmg}</strong> (d20:${r.base}${r.boonInfo}+${r.atkMod}=${r.total} vs Def ${gs.enemy.ac})${cl}`,r.crit?'crit':'dmg');
